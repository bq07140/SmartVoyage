...
6. 实现order_tickets函数（调用MCP服务器执行票务预订）
7. 定义Agent卡片（描述服务器能力和技能）
8. 创建TicketServer类（继承A2AServer）
9. 实现generate_sql_query方法（生成SQL查询或追问）
10. 实现handle_task方法（处理任务、识别意图、执行查询或预订）
11. 主函数（创建并运行服务器）
"""
import json
import asyncio
...
# 初始化LLM（用于预订，使用不同的temperature）
llm_order = ChatOpenAI(
    model=conf.model_name,
    base_url=conf.base_url,
    api_key=conf.api_key,
        ...
        async with streamablehttp_client("http://127.0.0.1:8003/mcp") as (read, write, _):
            ...
            async with ClientSession(read, write) as session:
                try:
                    ...
                    # 构建工具调用代理
                    agent = create_tool_calling_agent(llm_order, tools, prompt)
