...
if "agent_network" not in st.session_state:
    ...
    st.session_state.memory = ConversationMemory(short_term_limit=10)
...
# 两栏布局：左侧对话，右侧 Agent Card + Memory
...
with col2:
    ...
    with st.expander("短期对话", expanded=True):
        for msg in mem.short_term_messages[-5:]:
            role_label = "用户" if msg["role"] == "user" else "助手"
            st.markdown(f"**{role_label}**: {msg['content']}")

    with st.expander("用户偏好"):
        if mem.user_profile:
    ...
    with st.expander("当前任务"):
        if mem.current_task:
            for k, v in mem.current_task.items():
                st.markdown(f"**{k}**: {v}")
        else:
            st.markdown("无")

    with st.expander("查询历史"):
        if mem.entity_history:
            for entity in mem.entity_history[-5:]:
                st.markdown(f"**{entity['type']}**: {entity['query']}")
        else:
            st.markdown("无")

# 页脚
st.markdown("---")
st.markdown('<div class="footer">Powered by 黑马程序员 | 基于Agent2Agent的旅行助手系统 v3.0</div>', unsafe_allow_html=True)
